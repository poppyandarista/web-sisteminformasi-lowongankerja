...
if (isset($_GET['action']) && $_GET['action'] == 'hapus' && isset($_GET['id'])) {
  ...
  if (is_numeric($id)) {
    if ($db->hapus_kategori($id)) {
      $ajax_response['success'] = true;
      $ajax_response['message'] = 'Data kategori berhasil dihapus';
    } else {
      $ajax_response['message'] = 'Gagal menghapus data kategori';
    }
  } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'edit') {
  ...
  if ($db->update_kategori($id_kategori, $nama_kategori)) {
    $ajax_response['success'] = true;
    $ajax_response['message'] = 'Data kategori berhasil diperbarui';
  } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'tambah') {
  ...
  if ($db->tambah_kategori($nama_kategori)) {
    $ajax_response['success'] = true;
    $ajax_response['message'] = 'Data kategori berhasil ditambahkan';
  } else {
    $ajax_response['message'] = 'Gagal menambahkan data kategori';
  }
  echo json_encode($ajax_response);
  exit;
}
...
<head>
  ...
  <title>Data Kategori | LinkUp</title>
  ...
  <style>
    ...
    #modalTambahKategori:not(.hidden),
    #modalEditKategori:not(.hidden) {
      display: flex;
    }
    ...
    /* ==================== PERBAIKAN DATATABLES v2 UNTUK KATEGORI ==================== */
    .dt-length,
    .dt-search,
    .dt-info,
    .dt-paging {
    ...
    @media (max-width: 768px) {
      ...
      #kategoriTable_wrapper .dt-length,
      #kategoriTable_wrapper .dt-search {
        margin-bottom: 0.5rem !important;
      }

      #kategoriTable_wrapper .dt-info,
      #kategoriTable_wrapper .dt-paging {
        margin-top: 0.5rem !important;
      }
    }

    /* ==================== RESPONSIVE TABEL KATEGORI ==================== */
...
<body
  x-data="{ page: 'dataKategori', 'loaded': true, 'darkMode': false, 'stickyMenu': false, 'sidebarToggle': false, 'scrollTop': false, 'modalOpen': false }"
  x-init="
        ...
        <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
          ...
          <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div
              ...
              <h3 class="font-medium text-black dark:text-white">Tabel Data Kategori</h3>
              <button onclick="toggleModal('modalTambahKategori')"
                class="btn-tambah-kategori inline-flex items-center gap-2 text-xs font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600">
            ...
            <div class="p-4 sm:p-6 xl:p-7.5">
              ...
              <div class="table-wrapper overflow-x-auto">
                <table id="kategoriTable" class="w-full table-auto border-collapse text-left">
                  <thead>
                    <tr class="bg-gray-2 dark:bg-meta-4">
                      <th class="px-4 py-3 font-medium text-black dark:text-white">ID Kategori</th>
                      <th class="px-4 py-3 font-medium text-black dark:text-white">Nama Kategori</th>
                  ...
                  <tbody>
                    ...
                    if (!empty($data_kategori)) {
                      foreach ($data_kategori as $row) {
                        $format_id = "K" . sprintf("%04d", $row['id_kategori']);
                        ?>
                        <tr class="border-b border-[#eee] dark:border-strokedark hover:bg-gray-50 dark:hover:bg-meta-4">
                          ...
                          <td class="px-4 py-3">
                            <div class="kategori-card">
                              <div class="kategori-icon">
                                <?php echo strtoupper(substr($row['nama_kategori'], 0, 1)); ?>
                              </div>
                              <div>
                          ...
                          <td class="px-4 py-3">
                            <div class="flex gap-2">
                              <button
                                onclick="showEditForm(<?php echo $row['id_kategori']; ?>, '<?php echo addslashes($row['nama_kategori']); ?>')"
                              ...
                              <button
                                onclick="showDeleteConfirmation(<?php echo $row['id_kategori']; ?>, '<?php echo addslashes($row['nama_kategori']); ?>')"
                      ...
                      echo "<tr><td colspan='3' class='text-center py-4'>Tidak ada data kategori</td></tr>";
                    }
                    ?>
                  </tbody>
                </table>
    ...
    <div
      ...
      <div class="sticky top-0 z-10 bg-white dark:bg-boxdark border-b border-stroke dark:border-strokedark px-6 py-4">
        <div class="flex items-center justify-between">
          <div>
            <h5 class="text-lg font-bold text-black dark:text-white">Tambah Kategori Baru</h5>
            <p class="text-xs text-gray-500">Isi form untuk menambah data kategori pekerjaan.</p>
          </div>
      ...
      <div class="p-6 overflow-y-auto flex-1">
        <form id="formTambahKategori" action="" method="POST" class="space-y-4">
          <input type="hidden" name="action" value="tambah">
          <div>
            <label class="block text-sm font-medium mb-1 dark:text-white">Nama Kategori *</label>
  ...
  <div id="modalEditKategori"
    ...
    <div
      ...
      <div class="sticky top-0 z-10 bg-white dark:bg-boxdark border-b border-stroke dark:border-strokedark px-6 py-4">
        <div class="flex items-center justify-between">
          <div>
            <h5 class="text-lg font-bold text-black dark:text-white">Edit Kategori</h5>
            <p class="text-xs text-gray-500">Edit data kategori pekerjaan.</p>
          </div>
          <button onclick="hideAllModals()" class="text-gray-400 hover:text-red-500 transition-colors">
            <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              <path d="M18 6L6 18M6 6l12 12" />
      ...
      <div class="p-6 overflow-y-auto flex-1">
        <form id="formEditKategori" action="" method="POST" class="space-y-4">
    ...
    <div
      ...
      <div class="p-5">
        ...
        <div class="text-center mb-4">
          ...
          <p class="text-sm text-gray-600 dark:text-gray-400" id="hapusMessageKategori">
  ...
  <script>
    $(document).ready(function () {
      $('#kategoriTable').DataTable({
        ...
        "initComplete": function () {
          ...
          var $wrapper = $('#kategoriTable_wrapper');
    ...
    function hideAllModals() {
      const modals = ['modalTambahKategori', 'modalEditKategori', 'modalHapusKategori'];
    ...
    function showDeleteConfirmation(id, nama) {
      const message = document.getElementById('hapusMessageKategori');
      message.textContent = `Apakah Anda yakin ingin menghapus kategori "${nama}"?`;
      ...
      deleteLink.onclick = async function () {
        ...
        try {
          const response = await fetch(`datakategori.php?action=hapus&id=${id}`, {
            method: 'GET'
          });
          ...
          if (result.success) {
            showNotification('success', 'Data kategori berhasil dihapus!', 'Hapus Berhasil');
            toggleModal('modalHapusKategori', false);
            // Auto refresh setelah 2 detik
            setTimeout(() => {
              location.reload();
            }, 2000);
          } else {
            showNotification('error', result.message || 'Gagal menghapus data kategori', 'Hapus Gagal');
          }
        } catch (error) {
    ...
    // Handle form tambah kategori dengan AJAX
    document.getElementById('formTambahKategori').addEventListener('submit', async function(e) {
      ...
      try {
        const response = await fetch('datakategori.php', {
          method: 'POST',
          body: formData
        });
        ...
        if (result.success) {
          showNotification('success', 'Data kategori berhasil ditambahkan!', 'Tambah Berhasil');
          toggleModal('modalTambahKategori', false);
          this.reset();
          // Auto refresh setelah 2 detik
          setTimeout(() => {
            location.reload();
          ...
          showNotification('error', result.message || 'Gagal menambahkan data kategori', 'Tambah Gagal');
        }
      } catch (error) {
        console.error('Error:', error);
    ...
    // Handle form edit kategori dengan AJAX  
    document.getElementById('formEditKategori').addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (result.success) {
          showNotification('success', 'Data kategori berhasil diperbarui!', 'Edit Berhasil');
          toggleModal('modalEditKategori', false);
          // Auto refresh setelah 2 detik
          setTimeout(() => {
            location.reload();
          }, 2000);
